...
def check_phone_number:
  if (test("\\(\\+\\d{2}\\) \\d{3}-\\d{3}-\\d{3}")) then
    ...
    "Oops, it seems like I can't reach out to " + (match("([-0-9]+)") | .string) + "."
  end
;
...
def get_domains:
  [ match("(\\b[[:alnum:]-.]+\\.[[:alpha:]]{2,3})\\b"; "gi") | .string ]
;
...
def nice_to_meet_you:
  "Nice to meet you, " + (. | match("My name is ([[:alpha:]-]+).*"; "i") | .captures[0].string)
;
